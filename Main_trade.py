...
# Function to be used to get the forecast of data
...
if fetch_external_data:
    ...
    raw_data = partial(get_conversion_rate, sell_unit, buy_unit, **param_period)()
else:
...
for key, val in param_period.items():
    output_variables[key] = f"'{val}'"# 1. Import necessary modules and functions
import os
import time
import json
import pickle
...
if ASSET_TRADING:
    real_investment = TRADE_INVESTMENT*CLOSING_RATE*get_conversion_rate(buy_unit, currency_investment)
else:
    real_investment = 
